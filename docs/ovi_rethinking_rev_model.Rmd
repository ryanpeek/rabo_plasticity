...
m2<- map2stan(
  alist(
    DOWY ~ dpois(mu),
    log(mu) <- a + a_lag[lagEM] + bt7mx*temp_7_max,
    a ~ dnorm(0,10),
    a_lag[lagEM] ~ dnorm(a,sigma_lag),
    c(bt7mx) ~ dnorm(1,10),
    sigma_lag ~ dcauchy(0,1)
...
dens( post1$bdLev , col="red", add=F, ylim=c(0,50))
dens( post1$bt7mx, add=F)#ylim=c(0,1.5), xlim=c(-1.4,3.4))
dens( post1$bt30mn, col=rangi2 , lwd=2 , add=TRUE )
