...
d1 <- dplyr::select(rabo_logist, site, breed, lagEM, DOWY, everything(), 
    ...
    #filter(!site=="MFA") %>% 
  filter(site=="NFY" | site=="NFA" | site=="RUB") %>%
  as.data.frame
...
m1<- map2stan(
  alist(
    DOWY ~ dpois(mu),
    log(mu) <- a + a_lag[lagEM] + bt7mx*temp_7_max + bt30mn*temp_30_min + bQCV*Q_CV + bdLev*deltLevEX,
    a ~ dnorm(5,10),
    a_lag[lagEM] ~ dnorm(a, sigma_lag),
    c(bt7mx, bt30mn, bQCV, bdLev) ~ dnorm(1,10),
    sigma_lag ~ dcauchy(1,1)
),
data=d1, iter=6000 , chains=4, cores=2)
...
m2<- map2stan(
  alist(
    DOWY ~ dpois(mu),
    log(mu) <- a + a_site[site] + bt7mx*temp_7_max +
      bt7mxIND*(temp_7_max+Index),
    a ~ dnorm(0,10),
    a_site[site] ~ dnorm(a,sigma_site),
    c(bt7mx, bt7mxIND) ~ dnorm(1,10),
    sigma_site ~ dcauchy(0,2)
),
data=d1, iter=8000 , chains=4, cores=2)
...
m3 <- map2stan(
  alist(
    DOWY ~ dpois(mu),
    log(mu) <- a + a_lag[lagEM] + bt7mx*temp_7_max + bt30mn*temp_30_min +
      bQCV*Q_CV + bdLev*deltLevEX + bQmx*Q_max + bWairmx*W_air_max +
      bwy*WY,
    a ~ dnorm(10,10),
    a_lag[lagEM] ~ dnorm(a,sigma_lag),
    c(bt7mx, bt30mn, bQCV, bdLev, bQmx, bWairmx, bwy) ~ dnorm(1,10),
    sigma_lag ~ dcauchy(0,1)
),
data=d1, iter=8000 , chains=1, cores=2)

# plots and stuff
plot(m3)
dev.off()
precis(m3, depth=2, warn=F)
plot(precis(m3, depth=2,warn=F))

# show density of probabilities
post3 <- extract.samples(m3)
dens( post3$bdLev , col="red", add=F, ylim=c(0,50))
dens( post3$bt7mx, add=F)#ylim=c(0,1.5), xlim=c(-1.4,3.4))
dens( post3$bt30mn, col=rangi2 , lwd=2 , add=TRUE )
...
#start<-list(mu=mean(d1$DOWY), sigma=sd(d1$DOWY))

m4 <- map2stan(
  alist(
    DOWY ~ dnorm( mu , sigma),
    mu <- a + a_lag[lagEM] + bt7mx*temp_7_max + 
      bt30mn*temp_30_min + bQCV*Q_CV + bdLev*deltLevEX +
      bWair7*W_air_7_avg + bwyi*Index,
    a ~ dnorm( 215 , 100 ) ,
    a_lag[lagEM] ~ dnorm(a, sigma_lag),
    bt7mx ~ dnorm( 11 , 10 ) ,
    bt30mn ~ dnorm( 7 , 10 ) ,
...
m5 <- map2stan(
  alist(
    ...
    mu <- a + a_lag[lagEM] + bt7mx*temp_7_max + 
      bt30mn*temp_30_min + bQCV*Q_CV + bdLev*deltLevEX +
      bWair7*W_air_7_avg + bwyi*Index + bPrecip*days_no_ppt,
    a ~ dnorm( 215 , 100 ) ,
    a_lag[lagEM] ~ dnorm(a, sigma_lag),
    bt7mx ~ dnorm( 11 , 10 ) ,
    bt30mn ~ dnorm( 7 , 10 ) ,
...
plot( DOWY ~ temp_7_max , data=d1, pch=ifelse(d1$lagEM==1, 16, 21), col=ifelse(d1$WY==1, rangi2, "red"))
...
mu <- link( m4, 
            data=data.frame(temp_7_max=temp_7_max.seq, 
                            ...
                            Q_CV=1, deltLevEX=1, W_air_7_avg=1,
                            days_no_ppt=1, Index=10))
str(mu)

# use type="n" to hide raw data
plot( DOWY ~ temp_7_max , data=d1, pch=ifelse(d1$lagEM==1, 16, NA), col=ifelse(d1$WY==1, rangi2, "red")) #type="n")
