...
# Model 1: Varying Intercept for WY, breeding only
m1<- map2stan(
  alist(
    DOWY ~ dpois(mu),
    log(mu) <- a + a_wy[WY] + bt7mx*temp_7_max + bt30mn*temp_30_min + bQCV*Q_CV + bdLev*deltLev,
    a ~ dnorm(5,10),
    a_wy[WY] ~ dnorm(a, sigma_wy),
    c(bt7mx, bt30mn, bQCV, bdLev) ~ dnorm(1,10),
    sigma_wy ~ dcauchy(1,1)
),
data=d1s, iter=1e4, chains=4, cores=2)
...
plot( DOWY ~ temp_7_max , data=d1, pch=ifelse(d1$lagEM==1, 16, 21), col=ifelse(d1$WY==2011, rangi2, "red"), yaxt='n', ylab="")
