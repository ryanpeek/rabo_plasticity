...
Too much going on in the models above, so let's simplify and look at a few variables at a time across all sites, and see how predictions/models improve/decrease, make sure to include breeding and 
...
# only with full (lagged data)
# mS1S <- map2stan(
#   alist(
#     DOWY ~ dnorm( mu , sigma),
#     mu <- a + a_lag[lagEM] + bt7mxWY*(temp_7_max * Index)+
#       bB*breed + bReg*REG_id ,
#     a ~ dnorm(0, 10),
#     a_lag[lagEM] ~ dnorm(0, sigma_lag),
#     c(bB, bt7mxWY, bReg) ~ dnorm(0,10),
#     sigma ~ dnorm(0,10),
...
plot(mS1)
dev.off()
precis(mS1, depth=2, warn=F)
precis(mS1s, depth=2, warn=F)
precis(mS2, depth=2, warn=F)
precis(mS3, depth=2, warn=F)
plot(precis(mS1, depth=2))
...
```{r plotsSimp Counterfactual}
post1<-extract.samples(mS1s)
...
lines( temp_7_max.seq , mu1U.mean, lwd=2, col="darkblue" )
lines( temp_7_max.seq , mu1R.mean, lwd=2, col="maroon", lty=2 )
...
abline( a=(coef(mS1s)["a_wy[4]"]+coef(mS1s)["a"]), b=coef(mS1s)['bt7mx'])
