...
data1 <- add_WYD(data1, "Date") %>% 
  mutate(WYT = as.factor(WYT),
         DOY = as.integer(DOY),
         WY = as.integer(WY),
         DOWY = as.integer(DOWY))
names(data1)

# join just breeding points together:
# data2 <- inner_join(hydroDat, frogBreed, by = c("Date"="estim_strt", "site"="Site")) %>% 
#   mutate(site = as.factor(site),
#          WYT = as.factor(WYT))
# data2 <- add_WYD(data2, "Date") %>% 
#   mutate(WYT = as.factor(WYT),
#          DOY = as.integer(DOY),
#          WY = as.integer(WY),
#          DOWY = as.integer(DOWY))
...
nfdf <- data1 %>% #filter(site %in% sites) %>% 
  ...
  select(Date, site, obs_strt, totalEM, -WYT, DOY:DOWY, air.7:level.7dL, -ppt2_in, -days_wo_ppt2)

summary(nfdf)

# remove NAs and non-NF sites:
nfdf <- nfdf %>% filter(!is.na(air.7), !is.na(level.avg))
summary(nfdf)

# add binary 1 or 0 for breeding
nfdf <- nfdf %>% mutate(ovipos=as.integer(ifelse(is.na(totalEM), 0, 1)))
names(nfdf)
...
train <- d %>% sample_n(size=300, replace=FALSE) %>% row.names() %>% as.integer()

traindat <- d[train,]
testdat <- d[-train,]

#remove the dependent and identifier variables
traindat <- traindat %>% select(-ovipos, -river_id)
...
prin_comp$rotation[,1:3] %>% as.data.frame %>%
  ...
  select(vars, PC1:PC3) %>% arrange(PC1)

# loadings per variable PC2
prin_comp$rotation[,2:3] %>% as.data.frame %>%
  mutate(vars = row.names(prin_comp$rotation)) %>% 
  select(vars, PC2:PC3) %>% arrange(PC2)
...
#the proportion of var explained by each component (var / (sum total var))
